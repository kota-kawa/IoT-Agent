#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Jetson Orin Nano edge agent for the IoT server (CUDA + llama.cpp GPU).
...
structured JSON with a local model via llama.cpp (llama-cpp-python),
executes supported tasks on the Jetson, and reports the results back to
the server.

The implementation is hardware-aware only for the local LLM runtime,
using CUDA offload (n_gpu_layers) on Jetson Orin Nano.
"""

"""使う候補のモデル（GGUF 推奨・GPUで動かす前提）
...
# Jetson で動かすローカル LLM の推論設定（CUDA/GPU対応）
MODEL_PATH = os.getenv("LLAMA_MODEL_PATH", "Llama-3.2-3B-Instruct-Q3_K_M.gguf")

# CPUスレッド（トークナイズ等で使用）
# Jetson でも論理コア数に応じて増減させると良い
LLAMA_THREADS = int(os.getenv("LLAMA_THREADS", str(os.cpu_count() or 6)))
...
# 追加：GPU オフロード制御（-1 = 全層GPU推奨 / 0 = CPU / N = 先頭N層GPU）
LLAMA_N_GPU_LAYERS = int(os.getenv("LLAMA_N_GPU_LAYERS", "-1"))

# 追加：プロンプト処理バッチ（大きいほど速いがVRAM使用増）
LLAMA_N_BATCH = int(os.getenv("LLAMA_N_BATCH", "256"))

# 追加：物理バッチ（unified batch）。N_BATCH と同じか少し小さめが目安
LLAMA_N_UBATCH = int(os.getenv("LLAMA_N_UBATCH", "256"))

# 追加：llama.cpp の詳細ログ（0/1）。INFO ログを出したいときは環境変数 LLAMA_LOG_LEVEL=INFO も併用
LLAMA_VERBOSE = os.getenv("LLAMA_VERBOSE", "0").strip() in {"1", "true", "yes", "on"}
...
AGENT_ROLE_VALUE = "jetson-agent"           # 旧: raspberrypi-agent
AGENT_COMMAND_NAME = "agent_instruction"

# デバイスがネイティブにサポートするアクション定義
...
LLM_SYSTEM_PROMPT = (
    "You convert simple English instructions into JSON commands for a Jetson automation agent.\n"
...
def _load_device_id() -> str:
    ...
    new_id = f"jetson-agent-{uuid.uuid4().hex[:12]}"  # 旧: raspi-agent-...
    try:
...
def _create_llm() -> Llama:
    # 指定パスの GGUF モデルを読み込んで推論器インスタンスを生成（CUDA/GPU オフロード有効）
    ...
    logging.info(
        "llama.cpp params: threads=%s, n_ctx=%s, n_gpu_layers=%s, n_batch=%s, n_ubatch=%s",
        LLAMA_THREADS, LLAMA_CONTEXT, LLAMA_N_GPU_LAYERS, LLAMA_N_BATCH, LLAMA_N_UBATCH
    )

    # 重要: n_gpu_layers を指定（-1 = 全層GPU）。GPU で動かない場合は、llama-cpp-python を
    # CMAKE_ARGS='-DGGML_CUDA=on -DCMAKE_CUDA_ARCHITECTURES=87' でビルドし直してください。
    return Llama(
...
def _register_device(session: requests.Session, device_id: str) -> Tuple[bool, bool]:
    ...
    payload = {
        ...
        "meta": {
            ...
            "note": "llama.cpp CUDA-powered Jetson agent",
        },
    }
...
JOKES = [
    ...
    "My Jetson was down so I asked it about it and it said, 'The battery is low.'",
]
...
def main() -> None:
    ...
    llm = _create_llm()
    _console(
        "Model ready (path='{}', threads={}, context={}, n_gpu_layers={}, n_batch={}, n_ubatch={}).".format(
            MODEL_PATH,
            LLAMA_THREADS,
            LLAMA_CONTEXT,
            LLAMA_N_GPU_LAYERS,
