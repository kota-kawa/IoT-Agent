    ...
    return [
        {
            ...
            "content": (
                ...
                "回答は見やすいMarkdownで整理し、必要に応じて短い見出しや箇条書きを使ってください。"
            ),
        },
...
【出力フォーマット（見やすいMarkdown）】:
- 回答は原則として読みやすいMarkdownで記述してください。
- 要点が複数あるときは、短い見出し（##）や箇条書き（-）で整理してください。
- 手順説明が必要なときは、番号付きリスト（1. 2. 3.）を使ってください。
- コマンド名、パラメータ名、デバイスID、ファイル名は `バッククォート` で示してください。
- 1つの段落を長くしすぎず、適切に改行して視認性を確保してください。
- 見出し・本文・箇条書き・表の各行は必ず改行で分け、1行に詰め込まないでください。
- 表を使う場合は、ヘッダー行・区切り行・データ行をそれぞれ独立した行にしてください。
- 表は比較が必要な場合のみ使い、過剰な装飾は避けてください。
