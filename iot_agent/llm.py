...
async def _process_chat_with_tools(client: UnifiedClient, messages: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    system_prompt = (
        ...
        "To do this, you MUST use the 'control_device' tool. "
        "The 'command' argument for 'control_device' depends on the target device:\n"
        "- For Raspberry Pi 4, use command 'run_oled_robot_demo' with args: {'text': '...', 'duration': 5.0}.\n"
        "- For Jetson, use command 'show_text_on_oled' with args: {'text': '...', 'duration': 5.0}.\n"
        "If the user's request is ambiguous (e.g., 'Show hello on screen'), infer the correct device from context and use the appropriate command."
    )
